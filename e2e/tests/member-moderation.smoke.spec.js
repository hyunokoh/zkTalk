...
async function authedPost(request, path, token, data) {
...
async function joinPublicCommunity(request, communityId, token) {
    await authedPost(request, `/api/communities/${communityId}/join`, token);
}
async function waitForMemberRow(page, userId) {
    const row = page.locator(`[data-testid="member-row"][data-user-id="${userId}"]`);
    await expect(row).toBeVisible();
    return row;
}
async function assignRole(page, userId, role) {
...
async function moderateMember(page, userId, action, reason) {
    const row = await waitForMemberRow(page, userId);
    ...
    const dialog = page.locator(`[data-testid="member-action-confirm-dialog"][data-action-type="${action}"]`);
...
async function createAndJoin(request, label, communityId) {
    const session = await createDevAuthSession(request, label);
    await joinPublicCommunity(request, communityId, session.sessionToken);
    return session;
}
test('member moderation keeps moderator/admin boundaries clear in UI', async ({ browser, page, request, }) => {
    const seed = await getSeedData();
    const timestamp = Date.now();
    const moderatorUser = await createAndJoin(request, `p2-moderator-${timestamp}`, seed.communityId);
    const adminUser = await createAndJoin(request, `p2-admin-${timestamp}`, seed.communityId);
    const mutedUser = await createAndJoin(request, `p2-muted-${timestamp}`, seed.communityId);
    const kickedUser = await createAndJoin(request, `p2-kicked-${timestamp}`, seed.communityId);
    const bannedUser = await createAndJoin(request, `p2-banned-${timestamp}`, seed.communityId);
    await bootstrapAuthenticatedPage(page, seed.userA.sessionToken, `/communities/${seed.communitySlug}/settings/members`);
    ...
    const { context: moderatorContext, page: moderatorPage, } = await openAuthenticatedPage(browser, moderatorUser.sessionToken, `/communities/${seed.communitySlug}/moderation/reports`);
    const { context: adminContext, page: adminPage, } = await openAuthenticatedPage(browser, adminUser.sessionToken, `/communities/${seed.communitySlug}/settings/members`);
    try {
        ...
        await expect(moderatorPage
            .locator(`[data-testid="member-row"][data-user-id="${bannedUser.user.id}"]`)
            .getByTestId('member-role-select')).toHaveCount(0);
        ...
        await expect(adminPage
            .locator(`[data-testid="member-row"][data-user-id="${bannedUser.user.id}"]`)
            .getByTestId('member-role-select')).toBeVisible();
