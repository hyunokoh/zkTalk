...
test('community event can be created, RSVPed, and used to open a DM with an attendee', async ({
  browser,
  page,
}) => {
  ...
  const { context: attendeeContext, page: attendeePage } = await openAuthenticatedPage(
    browser,
  ...
  try {
    ...
    const attendeeEventCard = attendeePage.locator(
      `[data-testid="event-card"][data-event-id="${eventId}"]`,
    );
  ...
  const attendeesModal = page.locator(
    `[data-testid="event-attendees-modal"][data-event-id="${eventId}"]`,
  );
  await expect(attendeesModal).toBeVisible();

  const attendeeRow = attendeesModal.locator(
    `[data-testid="event-attendee-row"][data-user-id="${seed.userC.id}"][data-status="going"]`,
  );
