...
test('shared profile flow can send and accept a friend request', async ({
  browser,
  page,
  request,
}) => {
  ...
  const sharedProfileCard = page.locator(
    `[data-testid="friend-shared-profile-card"][data-profile-user-id="${addressee.user.id}"]`,
  );
  ...
  const { context: addresseeContext, page: addresseePage } = await openAuthenticatedPage(
    browser,
    addressee.sessionToken,
    '/friends',
  );
  ...
  try {
    ...
    const pendingRow = addresseePage.locator(
      `[data-testid="friend-row"][data-user-id="${requester.user.id}"][data-status="pending"][data-is-requester="false"]`,
    );
    ...
    const acceptedRow = addresseePage.locator(
      `[data-testid="friend-row"][data-user-id="${requester.user.id}"][data-status="accepted"]`,
    );
  ...
  const refreshedSharedProfileCard = page.locator(
    `[data-testid="friend-shared-profile-card"][data-profile-user-id="${addressee.user.id}"]`,
  );
  ...
  await expect
    ...
    .toBe('accepted');
  await expect(
    refreshedSharedProfileCard.getByTestId('friend-shared-profile-message-button'),
  ).toBeVisible();
});
