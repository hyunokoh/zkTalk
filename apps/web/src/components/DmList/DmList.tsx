...
export function DmList({ onConversationSelect }: DmListProps) {
  ...
  function getAvatar(row: ConversationRow) {
    ...
    return (
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gray-600 text-sm font-medium text-white">
        ?
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full w-64 flex-col border-r border-[#202225] bg-[#2f3136]" data-testid="dm-list">
      <div className="border-b border-[#202225] px-4 py-3 pl-12 md:pl-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h2 className="text-base font-semibold text-[#dcddde]">
              {t('dm.title')}
            </h2>
          ...
          <button
            ...
            className="inline-flex h-9 shrink-0 items-center justify-center rounded-full bg-indigo-600 px-3 text-xs font-semibold text-white transition hover:bg-indigo-500"
          >
            {t('dm.new')}
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading && (
          <div className="px-4 py-8 text-center text-sm text-[#72767d]">
            {t('common.loading')}
          </div>
        )}
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex min-w-0 flex-col gap-0.5">
                      ...
                      <span className="text-right text-[11px] text-[#72767d]">
                        {timeAgo}
                      </span>
                    </div>
                    ...
                    <div className="mt-0.5 flex items-center gap-1.5">
                      {row.unreadCount > 0 && !promotedTarget && (
                        <span className="inline-block rounded-full bg-indigo-600 px-1.5 py-0.5 text-[10px] font-medium text-white">
                          {row.unreadCount}
                        </span>
                      )}
                      {promotedTarget && (
                  ...
                  <div className="flex shrink-0 items-center gap-1.5">
                    {hasHistory && (
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-full border border-[#4f545c] bg-[#40444b] px-2.5 py-1 text-[11px] font-semibold text-[#dcddde] transition-colors hover:bg-[#4f545c]"
                      >
                        {t('dm.viewHistory')}
                        {row.unreadCount > 0 && (
                          <span className="inline-flex min-w-[1.15rem] items-center justify-center rounded-full bg-indigo-600 px-1 py-0.5 text-[10px] font-semibold text-white">
                            {row.unreadCount}
                          </span>
                        )}
                      </button>
                    ...
                    <button
                      ...
                      className="rounded-full border border-amber-500/40 bg-amber-500/10 px-2.5 py-1 text-[11px] font-semibold text-amber-400 transition-colors hover:bg-amber-500/20"
                    >
                      {t('dm.openChannel')}
                    </button>
                  </div>
