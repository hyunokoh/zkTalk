...
export function EmptyState({ title, description, action, className = '' }: EmptyStateProps) {
  return (
    <div className={`flex flex-col items-center justify-center rounded-[2rem] border border-line bg-bg-hover px-8 py-14 text-center text-fg-muted shadow-[0_24px_60px_rgba(2,8,23,0.18)] ${className}`.trim()}>
      ...
      {description ? <p className="mt-2 max-w-xl text-sm text-fg-muted">{description}</p> : null}
