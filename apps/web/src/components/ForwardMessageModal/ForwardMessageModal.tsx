...
export function ForwardMessageModal({ messageId, communityId, onClose }: ForwardMessageModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-bg-subtle" onClick={onClose}>
      <div
        className="w-full max-w-md rounded-lg border border-line bg-white shadow-xl dark:bg-bg-subtle"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b border-line px-4 py-3">
          <h3 className="text-sm font-semibold text-fg-muted">{t('forward.title')}</h3>
        ...
        <div className="max-h-80 overflow-y-auto p-4">
          {forwarded ? (
            <div className="flex flex-col items-center gap-2 py-8 text-center">
              ...
              <p className="text-sm text-fg-muted">{t('forward.success')}</p>
            </div>
          ) : isLoading ? (
            <div className="py-8 text-center text-sm text-fg-muted">{t('common.loading')}</div>
          ) : (
            <>
              <p className="mb-3 text-sm text-fg-subtle">{t('forward.selectChannel')}</p>
              <div className="space-y-1">
                {allChannels.map((channel) => (
                  <button
                    ...
                    className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm text-fg hover:bg-bg-hover-muted dark:hover:bg-bg-subtle"
                  >
